...
def test_important_source(demo_catalog: WebCatalog, demo_node_dataset: Node, demo_metadata: dict):
    """
    ...
    demo_node_dataset.add_attribrutes(demo_metadata[metadata_source], demo_catalog)
    for key in demo_metadata[metadata_source].keys():
        if key == cnst.EXTRACTORS_USED:
            continue
        assert hasattr(demo_node_dataset, key)
        assert getattr(demo_node_dataset, key) == demo_metadata[metadata_source][key]
    ...
    for key in demo_metadata[metadata_source].keys():
        if key == cnst.EXTRACTORS_USED:
            continue
        assert hasattr(demo_node_dataset, key)
        assert getattr(demo_node_dataset, key) == demo_metadata["metadata_source_important"][key]
...
def test_merge_sources(demo_catalog: WebCatalog, demo_node_dataset: Node,
                       demo_metadata: dict):
    """
    ...
    assert getattr(demo_node_dataset, key) == \
        list(set(demo_metadata["metadata_source_important"][key] +
...
def test_multiple_sources(demo_catalog: WebCatalog, demo_node_dataset: Node,
                          demo_metadata: dict):
    """
    ...
    demo_catalog.config = {
        "property_source": {
            "dataset": {
                ...
                "keywords": ["metadata_source_important", "metadata_source_random"],
                "authors": ["metadata_source_important", "metadata_source_random"],
            }
        }
    }
    demo_node_dataset.add_attribrutes(demo_metadata[metadata_source], demo_catalog)
...
def test_no_source_config(demo_catalog: WebCatalog, demo_node_dataset: Node,
                          demo_metadata: dict):
    """
