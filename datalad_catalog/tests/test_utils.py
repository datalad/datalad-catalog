...
def test_find_object_in_list():
    ...
    obj_found = find_duplicate_object_in_list(test_list, test_object_true, keys_to_match)
    assert obj_found == test_list[1]

def test_find_no_object_in_list():
    keys_to_match = test_object_false.keys()
    obj_found = find_duplicate_object_in_list(test_list, test_object_false, keys_to_match)
    assert obj_found is None

def test_find_object_in_list_singlekey():
    keys_to_match = ["source"]
    obj_found = find_duplicate_object_in_list(test_list, test_object_half_false, keys_to_match)
